...
from image_class_v3 import Image as Filter
from cnn_guesser_v3 import cnn_guesser
from report_v3 import createMultiPage
...
class SPAI(App):
    ...
    def build(self):
        ...
        av.add_widget(F.ActionButton(text='Report'.format(), on_press=self._reportpop))
    ...
    def _reportpop(self, obj):
        """
        Creates a popup to initiate the creation of a report in pdf format
        Args:
            obj: default, none
        ...
        # Defining the layout of the popup
        content = BoxLayout(orientation='vertical', spacing=7)

        reportpop = Popup(title='Add report details',
                          content=content,
                          size_hint=(0.6, 0.6))

        def title_f(instance, value):
            """
            Function to store the users input in a variable
            Args:
                instance: is the given instance of the Textinput
                value: the text that the user types in

            Returns:
                Returns: stores it in a global variable to be passed on to the function creating the report.
            """
            global title
            title = value

        def case_f(instance, value):
            """
            Function to store the users input in a variable
            Args:
                instance: is the given instance of the Textinput
        ...
        # Defining labels and input text boxes
        lbl1 = Label(text="Report title:")
        lbl2 = Label(text="Case name or number:")
        lbl3 = Label(text="Created by:")
        txt1 = TextInput()
        txt1.bind(text=title_f)
        txt2 = TextInput()
        txt2.bind(text=case_f)
        txt3 = TextInput()
        txt3.bind(text=author_f)

        # Adding labels and text input boxes to the popup
        content.add_widget(lbl1)
        content.add_widget(txt1)
        content.add_widget(lbl2)
        content.add_widget(txt2)
        content.add_widget(lbl3)
        content.add_widget(txt3)

        # 2 buttons are created for accept or cancel the current value
        btnlayout = BoxLayout(size_hint_y=None, height=50, spacing=5)
        btn = Button(text='Ok')
        btn.bind(on_release=lambda x: createMultiPage(title, case, author))
        btn.bind(on_release=reportpop.dismiss)
    ...
    def _queue_photos(self):
        ...
        try:
            ...
            pass
        print("Number of files to be filtered:",number_of_files)
    @staticmethod
    ...
    def _cnn(self, obj):
        ...
        try:
            print("Alex: Correct: {} Wrong: {} Accuracy: {}".format(accuracy_per[a_index],
                                                                    (accuracy_count[a_index] - accuracy_per[a_index]),
                                                                    (accuracy_per[a_index] / accuracy_count[a_index])))
            print("Gabbi: Correct: {} Wrong: {} Accuracy: {}".format(accuracy_per[g_index],
                                                                     (accuracy_count[g_index] - accuracy_per[g_index]),
                                                                     (accuracy_per[g_index] / accuracy_count[g_index])))
            print("Bjarke: Correct: {} Wrong: {} Accuracy: {}".format(accuracy_per[b_index],
                                                                      (accuracy_count[b_index] - accuracy_per[b_index]),
                                                                      (accuracy_per[b_index] / accuracy_count[
                                                                          b_index])))
            print("Monica: Correct: {} Wrong: {} Accuracy: {}".format(accuracy_per[m_index],
                                                                      (accuracy_count[m_index] - accuracy_per[m_index]),
                                                                      (accuracy_per[m_index] / accuracy_count[
                                                                          m_index])))
            print("Wenche: Correct: {} Wrong: {} Accuracy: {}".format(accuracy_per[w_index],
                                                                      (accuracy_count[w_index] - accuracy_per[w_index]),
                                                                      (accuracy_per[w_index] / accuracy_count[
                                                                          w_index])))
        except ZeroDivisionError:
            pass
        print()
        print(
            "Alex pic results: A:{} B:{} G:{} M:{} W:{}".format(alex_pic[a_index], alex_pic[b_index], alex_pic[g_index],
                                                                alex_pic[m_index], alex_pic[w_index]))
        print("Gabbi pic results: A:{} B:{} G:{} M:{} W:{}".format(gabbi_pic[a_index], gabbi_pic[b_index],
                                                                   gabbi_pic[g_index], gabbi_pic[m_index],
                                                                   gabbi_pic[w_index]))
        print("Bjarke pic results: A:{} B:{} G:{} M:{} W:{}".format(bjarke_pic[a_index], bjarke_pic[b_index],
                                                                    bjarke_pic[g_index], bjarke_pic[m_index],
                                                                    bjarke_pic[w_index]))
        print("Monica pic results: A:{} B:{} G:{} M:{} W:{}".format(monica_pic[a_index], monica_pic[b_index],
                                                                    monica_pic[g_index], monica_pic[m_index],
                                                                    monica_pic[w_index]))
        print("Wenche pic results: A:{} B:{} G:{} M:{} W:{}".format(wenche_pic[a_index], wenche_pic[b_index],
                                                                    wenche_pic[g_index], wenche_pic[m_index],
                                                                    wenche_pic[w_index]))
